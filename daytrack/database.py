...
Supports both local SQLite (for development) and Turso cloud via HTTP API (for production).

- If TURSO_DATABASE_URL is set → uses Turso HTTP API with cloud persistence
...
class DatabaseManager:
    """Database manager using local SQLite. Works everywhere, no Rust needed."""
    ...
    def remove_reminder(self, reminder_id: int, user_id: int) -> bool:
        ...
        if not row:
            return False
        flow_type, deleted_order = row["flow_type"], row["display_order"]
    ...
    def get_daily_plan(self, user_id: int, plan_date: str) -> Optional[dict]:
        row = self.conn.execute(
            "SELECT * FROM daily_plans WHERE user_id = ? AND plan_date = ?", (user_id, plan_date)).fetchone()
        return dict(row) if row else None
    ...
    def create_tasks(self, daily_plan_id: int, user_id: int, plan_date: str, tasks: List[dict]) -> None:
        ...
        self.conn.executemany(
            "INSERT INTO tasks (daily_plan_id, user_id, title, category, plan_date) VALUES (?, ?, ?, ?, ?)", rows)
        self.conn.commit()
    ...
    def get_weekly_summary(self, user_id: int, week_start: str) -> Optional[dict]:
        row = self.conn.execute(
            "SELECT * FROM weekly_summaries WHERE user_id = ? AND week_start = ?", (user_id, week_start)).fetchone()
        return dict(row) if row else None
    ...
    def get_random_memory(self, user_id: int) -> Optional[dict]:
        row = self.conn.execute(
            """SELECT moment, plan_date FROM daily_plans
               ...
               ORDER BY RANDOM() LIMIT 1""", (user_id,)).fetchone()
        return dict(row) if row else None
    ...
    def calculate_streak(self, user_id: int) -> int:
        rows = self.conn.execute(
            """SELECT plan_date FROM daily_plans
               ...
               ORDER BY plan_date DESC""", (user_id,)).fetchall()
        if not rows:
            return 0
        streak = 0
